...
//-----------------------------------------------------------------------------
void CCombineGolem::HeadcrabInit()
{
...
//-----------------------------------------------------------------------------
void CCombineGolem::Precache(void)
{
	BaseClass::Precache();
...
//-----------------------------------------------------------------------------
void CCombineGolem::CrawlFromCanister()
{
...
// Input  : NewActivity - 
//-----------------------------------------------------------------------------
void CCombineGolem::OnChangeActivity(Activity NewActivity)
{
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CCombineGolem::Classify(void)
{
	if (m_bHidden)
	{
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CCombineGolem::BodyTarget(const Vector& posSrc, bool bNoisy)
{
	Vector vecResult;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CCombineGolem::GetAutoAimRadius()
{
...
// Output : float
//-----------------------------------------------------------------------------
float CCombineGolem::MaxYawSpeed(void)
{
...
bool CCombineGolem::IsFirmlyOnGround()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCombineGolem::MoveOrigin(const Vector& vecDelta)
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CCombineGolem::ThrowAt(const Vector& vecPos)
{
...
// Input  : vecPos - 
//-----------------------------------------------------------------------------
void CCombineGolem::JumpToBurrowHint(CAI_Hint* pHint)
{
...
// Input  : vecVel - 
//-----------------------------------------------------------------------------
void CCombineGolem::Leap(const Vector& vecVel)
{
	SetTouch(&CCombineGolem::LeapTouch);
	...
	m_bMidJump = true;
	SetThink(&CCombineGolem::ThrowThink);
	SetNextThink(gpGlobals->curtime);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::ThrowThink(void)
{
	...
	if (GetFlags() & FL_ONGROUND)
	{
		SetThink(&CCombineGolem::CallNPCThink);
...
//			bThrown - 
//-----------------------------------------------------------------------------
void CCombineGolem::JumpAttack(bool bRandomJump, const Vector& vecPos, bool bThrown)
{
	Vector vecJumpVel;
	if (!bRandomJump)
	{
...
// Input  : *pEvent - 
//-----------------------------------------------------------------------------
void CCombineGolem::HandleAnimEvent(animevent_t* pEvent)
{
...
//-----------------------------------------------------------------------------
void CCombineGolem::SetBurrowed(bool bBurrowed)
{
	if (bBurrowed)
	{
		AddEffects(EF_NODRAW);
...
// Input  : *pTask - 
//-----------------------------------------------------------------------------
void CCombineGolem::RunTask(const Task_t* pTask)
{
	switch (pTask->iTask)
	{
...
//-----------------------------------------------------------------------------
bool CCombineGolem::HasHeadroom()
{
	trace_t tr;
...
// Input  : *pOther - 
//-----------------------------------------------------------------------------
void CCombineGolem::LeapTouch(CBaseEntity* pOther)
{
	m_bMidJump = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CCombineGolem::CalcDamageInfo(CTakeDamageInfo* pInfo)
{
...
//-----------------------------------------------------------------------------
void CCombineGolem::TouchDamage(CBaseEntity* pOther)
{
	CTakeDamageInfo info;
	CalcDamageInfo(&info);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CCombineGolem::GatherConditions(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::PrescheduleThink(void)
{
...
//-----------------------------------------------------------------------------
void CCombineGolem::EliminateRollAndPitch()
{
	...
	SetContextThink(&CCombineGolem::EliminateRollAndPitch, gpGlobals->curtime + TICK_INTERVAL, s_pPitchContext);
}
...
//-----------------------------------------------------------------------------
void CCombineGolem::BeginClimbFromCanister()
{
...
void CCombineGolem::JumpFromCanister()
{
	Assert(GetMoveParent());
...
void CCombineGolem::DropFromCeiling(void)
{
#ifdef HL2_EPISODIC
...
//-----------------------------------------------------------------------------
void CCombineGolem::PlayerHasIlluminatedNPC(CBasePlayer* pPlayer, float flDot)
{
	if (flDot < 0.97387f)
		return;
...
bool CCombineGolem::CanBeAnEnemyOf(CBaseEntity* pEnemy)
{
#ifdef HL2_EPISODIC
	if (IsHangingFromCeiling())
		return false;
...
// Input  : pTask - 
//-----------------------------------------------------------------------------
void CCombineGolem::StartTask(const Task_t* pTask)
{
	switch (pTask->iTask)
	{
	case TASK_HEADCRAB_WAIT_FOR_BARNACLE_KILL:
...
// Output :
//-----------------------------------------------------------------------------
float CCombineGolem::InnateRange1MinRange(void)
{
	return HEADCRAB_MIN_JUMP_DIST;
}

float CCombineGolem::InnateRange1MaxRange(void)
{
	return HEADCRAB_MAX_JUMP_DIST;
}

int CCombineGolem::RangeAttack1Conditions(float flDot, float flDist)
{
...
// Output :
//------------------------------------------------------------------------------
bool CCombineGolem::CorpseGib(const CTakeDamageInfo& info)
{
...
// Input  :
//------------------------------------------------------------------------------
void CCombineGolem::Touch(CBaseEntity* pOther)
{
...
// Output : 
//-----------------------------------------------------------------------------
int CCombineGolem::OnTakeDamage_Alive(const CTakeDamageInfo& inputInfo)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCombineGolem::ClampRagdollForce(const Vector& vecForceIn, Vector* vecForceOut)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCombineGolem::Event_Killed(const CTakeDamageInfo& info)
{
...
// Input  : Type - 
//-----------------------------------------------------------------------------
int CCombineGolem::TranslateSchedule(int scheduleType)
{
	switch (scheduleType)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CCombineGolem::SelectSchedule(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CCombineGolem::SelectFailSchedule(int failedSchedule, int failedTask, AI_TaskFailureCode_t taskFailCode)
{
...
//			*ptr - 
//-----------------------------------------------------------------------------
void CCombineGolem::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCombineGolem::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize, bool bCalledByLevelDesigner)
{
...
//-----------------------------------------------------------------------------
bool CCombineGolem::HandleInteraction(int interactionType, void* data, CBaseCombatCharacter* sourceEnt)
{
	...
	else if (interactionType == g_interactionVortigauntStompHit)
	{
		...
		CCombineGolem* pEntity = (CCombineGolem*)CreateEntityByName("npc_headcrab");
		pEntity->Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CCombineGolem::FValidateHintType(CAI_Hint* pHint)
{
	return true;
}
...
// Input  : &origin - 
//-----------------------------------------------------------------------------
void CCombineGolem::ClearBurrowPoint(const Vector& origin)
{
...
//-----------------------------------------------------------------------------
bool CCombineGolem::ValidBurrowPoint(const Vector& point)
{
	trace_t	tr;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::GrabHintNode(CAI_Hint* pHint)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CCombineGolem::FindBurrow(const Vector& origin, float distance, bool excludeNear)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::Burrow(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::Unburrow(void)
{
...
//-----------------------------------------------------------------------------
void CCombineGolem::InputUnburrow(inputdata_t& inputdata)
{
	if (IsAlive() == false)
		return;
...
//-----------------------------------------------------------------------------
void CCombineGolem::InputBurrow(inputdata_t& inputdata)
{
	if (IsAlive() == false)
		return;
...
//-----------------------------------------------------------------------------
void CCombineGolem::InputBurrowImmediate(inputdata_t& inputdata)
{
	if (IsAlive() == false)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::InputStartHangingFromCeiling(inputdata_t& inputdata)
{
	if (IsAlive() == false)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::InputDropFromCeiling(inputdata_t& inputdata)
{
	if (IsAlive() == false)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::CreateDust(bool placeDecal)
{
	trace_t	tr;
	AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1), GetAbsOrigin() - Vector(0, 0, 64), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::IdleSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::AlertSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::PainSound(const CTakeDamageInfo& info)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::DeathSound(const CTakeDamageInfo& info)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::TelegraphSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::AttackSound(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCombineGolem::BiteSound(void)
{
	EmitSound("NPC_HeadCrab.Bite");
}


LINK_ENTITY_TO_CLASS(npc_combine_golem, CCombineGolem);


AI_BEGIN_CUSTOM_NPC(npc_headcrab, CCombineGolem)
