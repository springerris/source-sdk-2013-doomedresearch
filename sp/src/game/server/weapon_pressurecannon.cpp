...
#define BOLT_WATER_VELOCITY	4000
...
//-----------------------------------------------------------------------------
// DR: pressure cannon bolt
//-----------------------------------------------------------------------------
class CCannonBolt : public CBaseCombatCharacter
{
	DECLARE_CLASS(CCannonBolt, CBaseCombatCharacter);
...
#ifdef MAPBASE
	static CCannonBolt* BoltCreate(const Vector& vecOrigin, const QAngle& angAngles, CBaseCombatCharacter* pentOwner = NULL);
...
#else
	static CCannonBolt* BoltCreate(const Vector& vecOrigin, const QAngle& angAngles, CBasePlayer* pentOwner = NULL);
#endif
...
LINK_ENTITY_TO_CLASS(cannon_bolt, CCannonBolt);

BEGIN_DATADESC(CCannonBolt)
// Function Pointers
...
IMPLEMENT_SERVERCLASS_ST(CCannonBolt, DT_CannonBolt)
END_SEND_TABLE()

#ifdef MAPBASE
CCannonBolt* CCannonBolt::BoltCreate(const Vector& vecOrigin, const QAngle& angAngles, CBaseCombatCharacter* pentOwner)
#else
CCannonBolt* CCannonBolt::BoltCreate(const Vector& vecOrigin, const QAngle& angAngles, CBasePlayer* pentOwner)
#endif
{
	// Create a new entity with CCannonBolt private data
	CCannonBolt* pBolt = (CCannonBolt*)CreateEntityByName("cannon_bolt");
...
//-----------------------------------------------------------------------------
bool CCannonBolt::CreateVPhysics(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
unsigned int CCannonBolt::PhysicsSolidMaskForEntity() const
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CCannonBolt::CreateSprites(void)
{
	// Start up the eye glow
	m_pGlowSprite = CSprite::SpriteCreate("sprites/yellowglow1.vmt", GetLocalOrigin(), false);
	m_pGlowTrail = CSpriteTrail::SpriteTrailCreate("sprites/yellowlaser1.vmt", GetLocalOrigin(), false);
	...
	if (m_pGlowTrail != NULL)
	{
		...
		m_pGlowTrail->SetTransparency(kRenderGlow, 255, 255, 255, 225, kRenderFxNoDissipation);
		m_pGlowTrail->SetLifeTime(0.5f);
		m_pGlowTrail->SetStartWidth(4);
		m_pGlowTrail->SetEndWidth(0.25f);
		m_pGlowTrail->SetScale(1);
		m_pGlowTrail->TurnOn();
	}
...
void CCannonBolt::Spawn(void)
{
	...
	SetTouch(&CCannonBolt::BoltTouch);

	SetThink(&CCannonBolt::BubbleThink);
...
void CCannonBolt::Precache(void)
{
	...
	PrecacheModel("sprites/yellowlaser1.vmt");
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCannonBolt::InputSetDamage(inputdata_t& inputdata)
{
...
//-----------------------------------------------------------------------------
void CCannonBolt::BoltTouch(CBaseEntity* pOther)
{
	...
	if (pOther->m_takedamage != DAMAGE_NO)
	{
		...
		if (GetOwnerEntity() && GetOwnerEntity()->IsPlayer() && pOther->IsNPC())
		{
			...
			if (pPlayer)
			{
				gamestats->Event_WeaponHit(pPlayer, true, "weapon_pressurecannon", dmgInfo);
			}
	...
	else
	{
		...
		if (pOther->GetMoveType() == MOVETYPE_NONE && !(tr.surface.flags & SURF_SKY))
		{
			...
			{
				SetThink(&CCannonBolt::SUB_Remove);
	...
	if (g_pGameRules->IsMultiplayer())
	{
		//		SetThink( &CCannonBolt::ExplodeThink );
		//		SetNextThink( gpGlobals->curtime + 0.1f );
	}
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCannonBolt::BubbleThink(void)
{
	QAngle angNewAngles;
...
//-----------------------------------------------------------------------------
// CWeaponPressureCannon
//-----------------------------------------------------------------------------

class CWeaponPressureCannon : public CBaseHLCombatWeapon
{
	DECLARE_CLASS(CWeaponPressureCannon, CBaseHLCombatWeapon);
public:

	CWeaponPressureCannon(void);
...
LINK_ENTITY_TO_CLASS(weapon_pressurecannon, CWeaponPressureCannon);

PRECACHE_WEAPON_REGISTER(weapon_pressurecannon);

IMPLEMENT_SERVERCLASS_ST(CWeaponPressureCannon, DT_WeaponPressureCannon)
END_SEND_TABLE()

BEGIN_DATADESC(CWeaponPressureCannon)
...
#ifdef MAPBASE
acttable_t	CWeaponPressureCannon::m_acttable[] =
{
...
IMPLEMENT_ACTTABLE(CWeaponPressureCannon);

acttable_t* GetPressureCannonActTable()
{
	return CWeaponPressureCannon::m_acttable;
}

int GetPressureCannonActtableCount()
{
	return ARRAYSIZE(CWeaponPressureCannon::m_acttable);
}
#endif
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CWeaponPressureCannon::CWeaponPressureCannon(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::Precache(void)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::PrimaryAttack(void)
{
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::SecondaryAttack(void)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPressureCannon::Reload(void)
{
	if (BaseClass::Reload())
	{
		m_bMustReload = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::Reload_NPC(bool bPlaySound)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::CheckZoomToggle(void)
{
	CBasePlayer* pPlayer = ToBasePlayer(GetOwner());
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::ItemBusyFrame(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::ItemPostFrame(void)
{
	// Allow zoom toggling
	CheckZoomToggle();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::FireBolt(void)
{
	if (m_iClip1 <= 0)
	{
		if (!m_bFireOnEmpty)
	...
	CCannonBolt* pBolt = CCannonBolt::BoltCreate(vecSrc, angAiming, pOwner);
...
//-----------------------------------------------------------------------------
inline void CWeaponPressureCannon::SetBolt(int iSetting)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::FireNPCBolt(CAI_BaseNPC* pOwner, Vector& vecShootOrigin, Vector& vecShootDir)
{
	Assert(pOwner);
	...
	CCannonBolt* pBolt = CCannonBolt::BoltCreate(vecShootOrigin, angAiming, pOwner);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPressureCannon::Deploy(void)
{
	if (m_iClip1 <= 0)
	{
#ifdef MAPBASE
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPressureCannon::Holster(CBaseCombatWeapon* pSwitchingTo)
{
	StopEffects();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::ToggleZoom(void)
{
	CBasePlayer* pPlayer = ToBasePlayer(GetOwner());
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::CreateChargerEffects(void)
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner());
...
// Input  : skinNum - 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::SetSkin(int skinNum)
{
	CBasePlayer* pOwner = ToBasePlayer(GetOwner());
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::DoLoadEffect(void)
{
	SetSkin(BOLT_SKIN_GLOW);
...
// Input  : state - 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::SetChargerState(ChargerState_t state)
{
...
//			*pOperator - 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::Operator_HandleAnimEvent(animevent_t* pEvent, CBaseCombatCharacter* pOperator)
{
	switch (pEvent->event)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::Operator_ForceNPCFire(CBaseCombatCharacter* pOperator, bool bSecondary)
{
...
//-----------------------------------------------------------------------------
bool CWeaponPressureCannon::SendWeaponAnim(int iActivity)
{
...
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::StopEffects(void)
{
	// Stop zooming
	if (m_bInZoom)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPressureCannon::Drop(const Vector& vecVelocity)
{
	StopEffects();
