...
public:
	...
	int		GetMinBurst() { return GetMaxClip1() - 1; }
	...
	float	GetFireRate(void) {
		CBaseEntity* pOw = GetOwnerEntity();
		if (pOw->IsPlayer()) {
			return MINIGUN_MAX_INTERVAL /2;
		}
		return MINIGUN_MAX_INTERVAL /2;
	}	// 13.3hz
	...
	{
		static const Vector cone = VECTOR_CONE_1DEGREES * 0.15;
		return cone;
	}
...
#ifdef MAPBASE
// Allows Weapon_BackupActivity() to access the M249's activity table.
acttable_t* GetM2491Acttable()
{
	return CWeaponM249::m_acttable;
}

int GetM249ActtableCount()
{
	return ARRAYSIZE(CWeaponM249::m_acttable);
}
#endif
...
void CWeaponM249::FireNPCPrimaryAttack(CBaseCombatCharacter* pOperator, Vector& vecShootOrigin, Vector& vecShootDir)
{
	...
	pOperator->FireBullets(3, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES,
...
#ifdef DEBUG
		DebugDrawLine(vecShootOrigin, vecShootOrigin + pOperator->EyeDirection3D() * 128, 255, 25, 25, false, 1);
#endif // DEBUG

		vecShootDir = pOperator->EyeDirection3D();
